<!--
 * @FilePath: \code\src\components\CodeBox.vue
 * @Version: 2.0
 * @LastEditors: lhl
 * @LastEditTime: 2022-07-29 16:21:27
 * @Description:
-->
<template>
  <div class="lf-code-box"
       :class="{'code-box-active':active}">
    <div class="demo-box">
      <slot name="demo">{{demo}}</slot>
    </div>
    <div class="message-box">
      <div class="message-title">{{messagetitle}}</div>
      <slot name="message">{{message}}</slot>
    </div>
    <lf-collapse :name=name
                 :time=0.1
                 :show="showCollapse"
                 style="border:none;border-left:3px solid #f0f0f0;border-right:3px solid #f0f0f0;">
      <template slot="head">
        <slot name="option">
          <div style="padding: 10px 20px;text-align:center;user-select:none;">
            <span v-if="codetitle">{{codetitle}}</span>
            <span v-else>
              <svg @click="click"
                   t="1658454598637"
                   class="icon actt"
                   viewBox="0 0 1024 1024"
                   version="1.1"
                   xmlns="http://www.w3.org/2000/svg"
                   p-id="663"
                   width="1.25em"
                   height="1.25em">
                <path d="M910.165333 519.509333H904.533333V254.805333a51.2 51.2 0 0 0-51.2-51.2H147.968a51.2 51.2 0 0 0-51.2 51.2v432.298667A39.765333 39.765333 0 0 0 136.533333 727.04h232.277334l-17.066667 63.146667h-21.845333a73.728 73.728 0 0 0-71.68 56.832h-41.984a17.066667 17.066667 0 0 0 0 34.133333h472.405333a39.765333 39.765333 0 0 0 39.594667 36.693333h181.930666a39.936 39.936 0 0 0 39.936-39.936V559.445333a39.936 39.936 0 0 0-39.936-39.936zM358.4 237.738667h494.933333a17.066667 17.066667 0 0 1 17.066667 17.066666v264.704h-142.165333a39.936 39.936 0 0 0-39.936 39.936v53.76H358.4z m329.898667 554.496a79.189333 79.189333 0 0 0-17.066667-2.048h-21.162667l-17.066666-63.146667h55.978666zM130.901333 254.805333a17.066667 17.066667 0 0 1 17.066667-17.066666H324.266667v375.466666H130.901333zM136.533333 692.906667a5.802667 5.802667 0 0 1-5.632-5.802667v-39.765333h557.397334v45.568z m477.866667 97.28H386.56l17.066667-63.146667H597.333333z m-284.672 34.133333h341.333333a38.570667 38.570667 0 0 1 17.066667 3.925333v18.773334H294.058667a39.765333 39.765333 0 0 1 35.84-22.698667z m586.069333 53.589333a5.802667 5.802667 0 0 1-5.802666 5.802667H728.234667a5.802667 5.802667 0 0 1-5.802667-5.802667V853.333333h193.536z m0-58.026666H722.432V559.445333a5.802667 5.802667 0 0 1 5.802667-5.802666h181.930666a5.802667 5.802667 0 0 1 5.802667 5.802666z"
                      fill="#3D3D63"
                      p-id="664"></path>
                <path d="M853.333333 576.512h-68.266666a17.066667 17.066667 0 0 0 0 34.133333h68.266666a17.066667 17.066667 0 0 0 0-34.133333zM284.501333 291.84H238.933333a17.066667 17.066667 0 0 0 0 34.133333h45.568a17.066667 17.066667 0 0 0 0-34.133333z"
                      fill="#3D3D63"
                      p-id="665"></path>
                <path d="M176.298667 308.906667m-17.066667 0a17.066667 17.066667 0 1 0 34.133333 0 17.066667 17.066667 0 1 0-34.133333 0Z"
                      fill="#3D3D63"
                      p-id="666"></path>
                <path d="M284.501333 382.805333H238.933333a17.066667 17.066667 0 0 0 0 34.133334h45.568a17.066667 17.066667 0 0 0 0-34.133334zM284.501333 439.637333H238.933333a17.066667 17.066667 0 0 0 0 34.133334h45.568a17.066667 17.066667 0 0 0 0-34.133334z"
                      fill="#3D3D63"
                      p-id="667"></path>
                <path d="M176.298667 399.872m-17.066667 0a17.066667 17.066667 0 1 0 34.133333 0 17.066667 17.066667 0 1 0-34.133333 0Z"
                      fill="#3D3D63"
                      p-id="668"></path>
                <path d="M753.834667 428.373333a74.069333 74.069333 0 1 0-74.069334-74.069333 74.069333 74.069333 0 0 0 74.069334 74.069333z m0-113.834666a39.936 39.936 0 1 1-39.936 39.765333 39.936 39.936 0 0 1 39.936-39.765333zM423.765333 384h102.4a17.066667 17.066667 0 0 0 0-34.133333h-102.4a17.066667 17.066667 0 0 0 0 34.133333zM575.488 406.698667h-151.722667a17.066667 17.066667 0 0 0 0 34.133333h151.722667a17.066667 17.066667 0 0 0 0-34.133333zM626.688 291.84H423.765333a17.066667 17.066667 0 0 0 0 34.133333h202.922667a17.066667 17.066667 0 0 0 0-34.133333zM517.632 542.037333h-93.866667a17.066667 17.066667 0 0 0 0 34.133334h93.866667a17.066667 17.066667 0 1 0 0-34.133334zM423.765333 519.338667h39.936a17.066667 17.066667 0 0 0 0-34.133334h-39.936a17.066667 17.066667 0 0 0 0 34.133334z"
                      fill="#3D3D63"
                      p-id="669"></path>
              </svg>
            </span>
          </div>
        </slot>
      </template>
      <template slot="artical">
        <slot v-if="showCode"
              name="codejs">
          <div style="padding: 5px 20px 20px 20px;">{{codejs}}</div>
        </slot>
        <slot v-else
              name="codehtml">
          <div style="padding: 5px 20px 20px 20px;">{{codehtml}}</div>
        </slot>
      </template>
    </lf-collapse>
  </div>
</template>

<script>
export default {
  name: 'CodeBox',
  props: {
    active: {
      default: false,
      type: Boolean
    },
    name: {
      default: 'demo1',
      required: true,
      type: [String, Number]
    },
    demo: {
      default: '组件实例'
    },
    message: {
      default: '组件描述'
    },
    codetitle: {
      default: ''
    },
    codejs: {
      default: '组件js代码'
    },
    codehtml: {
      default: '组件html代码'
    },
    messagetitle: {
      default: '描述类型'
    }
  },
  data () {
    return {
      showCode: true,
      showCollapse: true
    }
  },
  methods: {
    click () {
      this.showCollapse = !this.showCollapse
    }
  }
}
</script>
